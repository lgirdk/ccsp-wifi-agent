    ...
    )
{
    ...
    if (AnscEqualString(ParamName, "CurrentOperatingChannelBandwidth", TRUE))
    {
...
static int channels24G[] = {1, 6, 11};
static int channels5G[] = {36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116, 120, 124, 128, 132, 136, 140};
...
BOOL isDFSChannel(ULONG channel) {
    switch (channel) {
        case 52:
        case 56:
        case 60:
...
BOOL isWeatherChannel(ULONG channel) {
    switch (channel) {
        case 116:
        case 120:
        case 124:
...
ANSC_STATUS
GetInsNumsByWifiChannelWeight( ULONG* pWeight, ULONG *channel, BOOL *excludeDFS, COSA_DML_WIFI_CHAN_BW *bandwidth)
{
    PCOSA_DATAMODEL_WIFI        pWiFi       = (PCOSA_DATAMODEL_WIFI)g_pCosaBEManager->hWifi;
    PCOSA_DML_WIFI_RADIO            pWifiRadio      = NULL;
    ...
    )
{
    ...
    if (AnscEqualString(ParamName, "ChannelWeight", TRUE))
    {
    ...
    )
{
    ...
    /* check the parameter name and return the corresponding value */
    if( AnscEqualString(ParamName, "WiFiDisableBasicRates", TRUE))
    {
        /* collect value */
    ...
    /* check the parameter name and return the corresponding value */
    if( AnscEqualString(ParamName, "WiFiDisableSupportedRates", TRUE))
    {
        /* collect value */
...
int CheckStringIsHexString(char *value)
{
    char buff[16];
    int len = 0;
    int i = 0;
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "X_COMCAST-COM_WEPKey", TRUE))
    {
        if (pWifiApSec->Cfg.ModeEnabled == COSA_DML_WIFI_SECURITY_WEP_64 )
        {
    ...
    if(AnscEqualString(ParamName, "X_COMCAST-COM_KeyPassphrase", TRUE))
    {
	//XH 5G
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "X_LGI-COM_ActivatePushButton", TRUE))
    {
        /* collect value */
    ...
    if( AnscEqualString(ParamName, "X_LGI-COM_CancelSession", TRUE))
    {
        /* collect value */
    ...
    )
{
    ...
    if (AnscEqualString(ParamName, "X_LGI-COM_Pin", TRUE))
    {
    ...
    if (AnscEqualString(ParamName, "X_LGI-COM_ClientPin", TRUE))
    {
    ...
    // LGI ADD - START
    if(AnscEqualString(ParamName, "X_LGI-COM_WpsStatus", TRUE))
    {
	char strbuf[64] = {0};
        CosaDmlWiFi_getWpsStatus((pWifiAp->AP.Cfg.InstanceNumber -1), strbuf);
        if ( AnscSizeOfString(strbuf) < *pUlSize)
        {
            AnscCopyString(pValue, strbuf);
            return 0;
        }
        else
        {
           *pUlSize = AnscSizeOfString(strbuf)+1;
           return 1;
        }
    }
    // LGI ADD - END
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "X_COMCAST-COM_BytesSent", TRUE) || ( AnscEqualString(ParamName, "BytesSent", TRUE))) // LGI ADD
    {
        /* collect value */
    ...
    if( AnscEqualString(ParamName, "X_COMCAST-COM_BytesReceived", TRUE)  || ( AnscEqualString(ParamName, "BytesReceived", TRUE))) // LGI ADD
    {
        /* collect value */
...
//    PCOSA_DML_WIFI_RadiusSetting  pRadiusSetting   = (PCOSA_DML_WIFI_RadiusSetting)hInsContext;
   PCOSA_DATAMODEL_WIFI            pMyObject    = (PCOSA_DATAMODEL_WIFI     )g_pCosaBEManager->hWifi;
    ...
    if( AnscEqualString(ParamName, "ReAuthInterval", TRUE))
    {
        /* collect value */
    ...
    )
{
    ...
    {
        /* save update to backup */
        if(iValue != pWifiAp->AP.RadiusSetting.iReAuthInterval)
        {
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "LocationData", TRUE))
    {
        /* collect value */
    ...
    )
{
    ...
    if ( pCosaContext->bNew )
    {
        ...
        if( ANSC_STATUS_SUCCESS != CosaDmlMacFilt_AddEntry(pWiFiAP->Cfg.InstanceNumber, pMacFilt) )
    	{
			...
			len = sprintf( table_name, "Device.WiFi.AccessPoint.%d.X_CISCO_COM_MacFilterTable.%d.", pWiFiAP->Cfg.InstanceNumber, pMacFilt->InstanceNumber);
