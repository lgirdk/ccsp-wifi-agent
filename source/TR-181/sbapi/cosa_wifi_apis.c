...
static void MeshNotifySecurityChange(INT apIndex, PCOSA_DML_WIFI_APSEC_CFG pStoredApSecCfg)
{
    ...
    switch (pStoredApSecCfg->ModeEnabled)
    {
        ...
        case COSA_DML_WIFI_SECURITY_WPA3_Personal:
            ...
            break;
        case COSA_DML_WIFI_SECURITY_WPA2_WPA3_Personal:
...
#if defined(DUAL_CORE_XB3) || defined(CISCO_XB3_PLATFORM_CHANGES) || defined (_XB6_PRODUCT_REQ_) || defined (_COSA_BCM_MIPS_) || defined (_HUB4_PRODUCT_REQ_) || defined(_LG_MV1_CELENO_)
    pthread_t tid4;
#endif
...
wifiDbgPrintf("%s pSsid = %s\n",__FUNCTION__, pSsid);
    ...
    pEntry->Info.ModesSupported = COSA_DML_WIFI_SECURITY_None | 
				  ...
				  COSA_DML_WIFI_SECURITY_WPA2_WPA3_Personal |
...
wifiDbgPrintf("%s pSsid = %s\n",__FUNCTION__, pSsid);
    ...
    {
	    pCfg->ModeEnabled = COSA_DML_WIFI_SECURITY_WPA2_WPA3_Personal;
    } else if (strncmp(securityType,"11i", strlen("11i")) == 0)
    {
        if(strncmp(authMode,"EAPAuthentication", strlen("EAPAuthentication")) == 0)
        {
...
wifiDbgPrintf("%s\n",__FUNCTION__);
    ...
    if (pCfg->ModeEnabled != pStoredCfg->ModeEnabled) {
		...
		if (pCfg->ModeEnabled == COSA_DML_WIFI_SECURITY_None) 
       {
        ...
        } else if (pCfg->ModeEnabled == COSA_DML_WIFI_SECURITY_WPA2_WPA3_Personal)
        {
            strcpy(securityType,"11iand11s");
            strcpy(authMode,"PSKAuthentication");
            CcspWifiEventTrace(("RDK_LOG_NOTICE, Wifi security mode WPA2-WPA3-Personal is Enabled\n"));
            CcspWifiTrace(("RDK_LOG_WARN,RDKB_WIFI_CONFIG_CHANGED : Wifi security mode WPA2-WPA3-Personal is Enabled\n"));
