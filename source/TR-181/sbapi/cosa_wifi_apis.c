...
int getdefaultssidfromnvam(char *SSID)
{
    char *buff = NULL;
    char prefix[32] = {0};
    char random[32] = {0};

    syscfg_executecmd(__func__,"echo `rpcclient 192.168.254.253 \'nvread O.g.1 str\' | tail -n2`", &buff);
    if (buff != NULL ){
        snprintf(prefix,sizeof(prefix),buff);
        free(buff);
        buff = NULL;
    } else {
        return 0;
    ...
    syscfg_executecmd(__func__,"echo `rpcclient 192.168.254.253 \'nvread O.g.14 str\' | tail -n2`", &buff);
    if (buff != NULL ){
        snprintf(random,sizeof(random),buff);
        free(buff);
        buff = NULL;
    } else {
        return 0;
    ...
    snprintf(SSID,COSA_DML_WIFI_MAX_SSID_NAME_LEN-1,"%s%s",prefix,random);
    return 1;
}
#endif
...
ANSC_STATUS
getRadiusTransportInterface(int *radiusInterface)
{
    int retVal = CCSP_SUCCESS;
    char *strValue  = NULL;
...
void checkforbiddenSSID(int index)
{
    ...
    if (retPsmGet == CCSP_SUCCESS)
    {
        if(isReservedSSID(strValue, SSID))
        {
            ...
            if(getdefaultssidfromnvam(SSID)){
...
CosaDmlWiFi_GetRapidReconnectThresholdValue(ULONG vAPIndex, int	*rapidReconnThresholdValue )
{
	...
	/*
	   struct _COSA_DML_WIFI_RADIO_CFG is packed, so we can't rely on the elements
	   within it being correctly aligned. Therefore memcpy into rapidReconnThresholdValue,
	   which is always safe to do.
	*/
	memcpy (rapidReconnThresholdValue, &intValue, sizeof(*rapidReconnThresholdValue));

	return (retPsmGet == CCSP_SUCCESS) ? ANSC_STATUS_SUCCESS : ANSC_STATUS_FAILURE;
}
...
)
{
    ...
    sprintf(pInfo->X_LGI_COM_Pin, "%08d", pin);
...
#if !defined(_XF3_PRODUCT_REQ_) && !defined(_CBR_PRODUCT_REQ_) && !defined(_HUB4_PRODUCT_REQ_) && !defined (_XB6_PRODUCT_REQ_) && !defined (_COSA_BCM_ARM_) && !defined (_ARRIS_XB6_PRODUCT_REQ_) && !defined(_XB7_PRODUCT_REQ_) && !defined(_PLATFORM_TURRIS_) && !defined(_LG_MV1_CELENO_)
