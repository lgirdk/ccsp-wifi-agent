...
/* Legacy Devices Like XB3 have systemd on the side with WiFi Agent, but don't use Service Files */
    ...
    // DocsisIf_StartDocsisManager();
        ...
        sprintf(rpcCmd, "/usr/bin/rpcclient2 \"syscfg get X_RDKCENTRAL-COM_LoggerEnable\" | grep -v \"RPC\"");
        ...
        memset(out, 0, sizeof(out));
        sprintf(rpcCmd, "/usr/bin/rpcclient2 \"syscfg get X_RDKCENTRAL-COM_LogLevel\" | grep -v \"RPC\"");
        _get_shell_output(rpcCmd, out, sizeof(out));
        if( NULL != out )
        {
        ...
        memset(out, 0, sizeof(out));
        sprintf(rpcCmd, "/usr/bin/rpcclient2 \"syscfg get X_RDKCENTRAL-COM_WiFi_LogLevel\" | grep -v \"RPC\"");
        _get_shell_output(rpcCmd, out, sizeof(out));
        if( NULL != out )
        {
        ...
        memset(out, 0, sizeof(out));
        sprintf(rpcCmd, "/usr/bin/rpcclient2 \"syscfg get X_RDKCENTRAL-COM_WiFi_LoggerEnable\" | grep -v \"RPC\"");
        _get_shell_output(rpcCmd, out, sizeof(out));
        if( NULL != out )
        {
